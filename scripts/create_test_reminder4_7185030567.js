...
//   2. INSERT reminder (short_anti_tobacco, scheduled через 2 мин) или UPDATE existing scheduled/sent reminder
// Не трогает других пользователей. Не делает seed/reset/migration.

const db = require('../src/db/connection');

const TELEGRAM_ID = 7185030567;
const CONFIRM_FLAG = '--confirm-create-test-reminder4-7185030567';

async function run() {
  if (!process.argv.includes(CONFIRM_FLAG)) {
    console.log('Скрипт не выполнен — требуется подтверждение.');
    console.log('Для создания тестового reminder запусти:');
    console.log('  node scripts/create_test_reminder4_7185030567.js ' + CONFIRM_FLAG);
    return;
  }
  ...
  if (progress.current_step_number !== 4) {
    console.error(
      `Неожиданное состояние: step=${progress.current_step_number}`
    );
    console.error('Ожидалось: step=4. Остановлено.');
    return;
  }
  if (progress.next_procedure_type !== 'short_anti_tobacco') {
    console.error(
      `Неожиданное состояние: next=${progress.next_procedure_type}`
    );
    console.error('Ожидалось: next=short_anti_tobacco. Остановлено.');
    return;
  }
  if (progress.last_completed_procedure_type !== 'short_quick_lever') {
    console.error(
      `Неожиданное состояние: last_completed=${progress.last_completed_procedure_type}`
    );
    console.error('Ожидалось: last_completed=short_quick_lever. Остановлено.');
    return;
  }

  // 3. Найти последнюю completed counted short_quick_lever сессию
  const lastSession = await db('procedure_sessions as ps')
    ...
    .where('p.procedure_type', 'short_quick_lever')
    .select('ps.id', 'ps.procedure_id', 'p.procedure_type')
    .orderBy('ps.id', 'desc')
    .first();

  if (!lastSession) {
    console.error('Последняя completed counted short_quick_lever сессия не найдена. Остановлено.');
    return;
  }
  console.log('lastSession.id:', lastSession.id, '| procedure_type:', lastSession.procedure_type);

  // 4. Найти процедуру short_anti_tobacco
  const shortAntiTobacco = await db('procedures').where({ procedure_type: 'short_anti_tobacco' }).first();
  if (!shortAntiTobacco) {
    console.error('Процедура short_anti_tobacco не найдена в таблице procedures. Остановлено.');
    return;
  }
  ...
  await db.transaction(async (trx) => {
    ...
    if (existing) {
      ...
      await trx('reminders').insert({
        user_id: user.id,
        procedure_id: shortAntiTobacco.id,
