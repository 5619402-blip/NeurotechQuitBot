// READ-ONLY аудит пользователя 7185030567. Только SELECT — никаких INSERT/UPDATE/DELETE.
...
async function run() {
  ...
  row('rules_video_watched_at', user.rules_video_watched_at);
  ...
  sep('18. GIFT_ACCESS_TOKENS (created/owned)');
  const giftOwned = await db('gift_access_tokens').where({ created_by_admin_id: uid }).catch(() => []);
  if (!giftOwned.length) { console.log('  Созданных gift-токенов нет.'); }
  else giftOwned.forEach((g, i) => {
    console.log('  [' + i + '] id=' + g.id + ' token=' + g.token.slice(0, 8) + '...');
    row('    gift_access_type', g.gift_access_type);
    row('    status', g.status);
    row('    activated_by_user_id', g.activated_by_user_id);
  ...
  const summary = {
    ...
    events: ev.length,
    'gift_tokens (activated_by)': gift.length,
    'gift_tokens (created_by)': giftOwned.length,
  };
  let total = 0;
